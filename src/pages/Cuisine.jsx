...
function Cuisine() {
    ...
    const getCuisine = async (name) => {
        let data
        if(name === 'dessert'){
            data = await fetch(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${process.env.REACT_APP_API_KEY}&type=${name}&number=12&addRecipeInformation=true`)
        }else{
            data = await fetch(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${process.env.REACT_APP_API_KEY}&cuisine=${name}&number=12&addRecipeInformation=true`)
        }
